...
if ($compiledArtifactFolder -ne "none"){
  ...
  write-output "report Findings pdf-detailed `"$aseAppName-$BuildNumber.pdf`" `"$aseAppName-$BuildNumber.ozasmt`" -includeSrcBefore:5 -includeSrcAfter:5 -includeTrace:definitive -includeHowToFix" >> script.scan
